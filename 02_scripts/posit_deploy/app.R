...
raw <- read.csv("ged_2025.csv", stringsAsFactors = FALSE) |>
  mutate(
    date          = as.Date(date),
...
build_month_plot <- function(m, show_all_conflicts = FALSE, highlight_country = NULL) {
  ...
  ggplot() +
    ...
    geom_text(
      data  = country_nodes,
      aes(x = lx, y = ly, label = country, angle = text_angle, hjust = hjust),
      size = 3, colour = "grey20"
    ) +
    annotate("text",
      x = label_radii, y = 0,
      label = paste0("Day ", label_days),
      size = 3.0, colour = "grey20", hjust = 0, vjust = 0.5
    ) +
    coord_fixed(
      ratio = 1, clip = "off",
